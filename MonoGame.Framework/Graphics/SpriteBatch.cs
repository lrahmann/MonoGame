...
namespace Microsoft.Xna.Framework.Graphics
{
	...
	public class SpriteBatch : GraphicsResource
	{
            ...
            float rotation, Vector2 origin, Vector2 scale, SpriteEffects effects, float layerDepth, List<Texture2D> images, Color imageColor = default(Color))
        {
            ...
            int crtImageIdx = 0;
            //var defaultGlyph = spriteFont.Glyphs[spriteFont.DefaultCharacter.Value];
            ...
            //fixed (SpriteFont.Glyph* pGlyphs = spriteFont.Glyphs)
                ...
                if (!spriteFont.Glyphs.ContainsKey(c))
                {
                    c = spriteFont.DefaultCharacter.Value;
                }

                var pCurrentGlyph = spriteFont.Glyphs[c];
                ...
                if (firstGlyphOfLine)
                    ...
                    if (c >= '\uE000' && c <= '\uE004')    // if we encounter a placeholder-char then draw the next image
                    {
                        ...
                        float lineWidth = (pCurrentGlyph.WidthIncludingBearings);

                        float buttonMaxHeight = pCurrentGlyph.Cropping.Height;
                        float charHeight = pCurrentGlyph.BoundsInTexture.Height;
